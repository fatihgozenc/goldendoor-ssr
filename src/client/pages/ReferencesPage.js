					...
					<a key={key} className="home__route" href={route.lang === 'en' ? `/en/referenzen/${route.slug}` : `/referenzen/${route.slug}`}>
						<div className="home__route--wrapper">
							...
							<div className="home__route--content">
								...
								<p>{route.excerpt}</p>
								<span className="home__route--link">{route.title} →</span>
							</div>
						</div>
					</a>
				))}
				<div className="pool__pagination">
					{
						pageCount.map((item,key) => { 
							if (key === 1) {
								return <a key={key} href={data.lang === 'en' ? `/en/referenzen` : `/referenzen`}>{key}</a>
							} else if ( key > 1) {
								return <a key={key} href={data.lang === 'en' ? `/en/referenzen/page/${key}` : `/referenzen/page/${key}`}>{key}</a>
							} else {
								return null;
							}
						})
